...
export default class schism {
  constructor({target = target, mutation = 'words', overflow = 'hidden'}) {
    this.target = target
    this.mutation = mutation
    this.original = target.cloneNode()
  ...
  split() {
    // initially split into chars and work up from there
  ...
  init() {
    ...
    this.splitWords.forEach(w => {
      ...
      if (hyphenSplit.length > 1) {
        hyphenSplit.forEach((h, i) => {
          const wordInnerSpan = this.innerSpan.cloneNode()
          wordInnerSpan.innerHTML = h
      ...
      if (children.length > 0) {
        children.forEach(c => {
          ...
          if (childNode.innerHTML !== '&amp;') {
            splitWord = childNode.innerHTML.split('')
          } else {
            splitWord = ['&']
          }
